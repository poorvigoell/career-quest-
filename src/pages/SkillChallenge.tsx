...
const SkillChallenge = () => {
  ...
  const [timeRemaining, setTimeRemaining] = useState(challenge?.timeLimit * 60 || 0); // in seconds
  ...
  const { title, emoji, category, description, timeLimit, skillsRequired, instructions, submissionType } = challenge;
                ...
                {submissionType === "text" ? (
                  <div className="mb-4">
                    <textarea
                      ...
                      className="w-full h-64 p-3 border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50"
                      value={submission}
                  ...
                  <div className="mb-4">
                    <div className="border-2 border-dashed border-border rounded-lg p-8 text-center">
                      {file ? (
                        <div>
                          <p className="mb-2 text-primary font-medium">{file.name}</p>
                        ...
                        <>
                          <p className="mb-2 text-lg font-medium">Drop your files here or</p>
                ...
                <div className="flex justify-end">
                  <button
                    ...
                    className={`btn-primary flex items-center gap-2 ${
                      (submissionType === "text" ? !submission : !file) ? "opacity-50 cursor-not-allowed" : ""
                    }`}
                  >
                    {isSubmitting ? (
                      <>
          ...
          <div>
            <div className="bg-white dark:bg-card shadow-md rounded-lg p-5">
              ...
              <ul className="space-y-3 text-sm">
                ...
                <li className="flex gap-2">
                  <span className="font-bold text-primary">•</span>
                  <span>Manage your time well - don't spend too long on any one aspect.</span>
                </li>
                <li className="flex gap-2">
                  <span className="font-bold text-primary">•</span>
                  <span>Focus on quality over quantity in your submission.</span>
                </li>
                <li className="flex gap-2">
                  <span className="font-bold text-primary">•</span>
                  <span>Don't worry about perfection - this is about learning!</span>
                </li>
              </ul>
              
              <div className="mt-6 pt-4 border-t border-border">
                <h4 className="font-medium mb-2">Need more help?</h4>
                <button className="text-primary hover:underline text-sm">View resources</button>
              </div>
            </div>
          </div>
        </div>
